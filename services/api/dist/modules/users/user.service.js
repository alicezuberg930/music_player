...
const password_1 = require("@yukikaze/lib/password");
...
const email_1 = __importDefault(require("@yukikaze/email"));
class UserService {
    ...
    async signIn(request, response) {
        try {
            ...
            if (!isPasswordValid)
                throw new exception_1.BadRequestException('Invalid password');
            // Generate JWT access token
            const token = await new jwt_1.JWT(create_env_1.env.JWT_SECRET).sign({ id: user.id }, { expiresIn: create_env_1.env.JWT_EXPIRES_IN });
            response.cookie('accessToken', token, {
                httpOnly: true,
                secure: create_env_1.env.NODE_ENV === "production", // Required for HTTPS
                sameSite: create_env_1.env.NODE_ENV === "production" ? 'lax' : 'strict', // Required for cross-domain cookies
                domain: create_env_1.env.NODE_ENV === "production" ? '.tien-music-player.site' : undefined, // Share cookie across subdomains
                maxAge: create_env_1.env.JWT_EXPIRES_IN * 1000 // 1 day
            });
    ...
    async signUp(request, response) {
        try {
            ...
            if (existingUser)
                throw new exception_1.BadRequestException('Email is already registered');
            const hashedPassword = await new password_1.Password().hash(password);
            const verifyToken = await new password_1.Password().hash((0, create_cuid_1.createId)());
            ...
            const verifyLink = `${create_env_1.env.NODE_ENV === "production" ? 'https://tien-music-player.site' : 'http://localhost:5173'}/verify/${user[0].id}?token=${verifyToken}`;
            (0, email_1.default)({
                to: email,
            ...
            const token = await new jwt_1.JWT(create_env_1.env.JWT_SECRET).sign({ id: user[0].id }, { expiresIn: create_env_1.env.JWT_EXPIRES_IN });
            response.cookie('accessToken', token, {
                httpOnly: true,
                secure: create_env_1.env.NODE_ENV === "production", // Required for HTTPS
                sameSite: create_env_1.env.NODE_ENV === "production" ? 'lax' : 'strict', // Required for cross-domain cookies
    ...
    async myProfile(request, response) {
        try {
            if (!request.userId)
                throw new exception_1.BadRequestException('User ID is missing in request');
            const data = await db_1.db.query.users.findFirst({
    ...
    async verifyEmail(request, response) {
        try {
            ...
            if (user.verifyToken && user.verifyToken !== token)
                throw new exception_1.BadRequestException('Invalid verification token');
            if (user.verifyTokenExpires && user.verifyTokenExpires < new Date())
                throw new exception_1.BadRequestException('Verification token has expired');
